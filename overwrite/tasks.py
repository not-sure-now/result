...
import neural_compressor.training
from neural_compressor.training import prepare_compression

import torch
import torch.nn as nn

from ultralytics.nn.modules import (C_ADD, AIFI, C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
...
class BaseModel(nn.Module):
    ...
    def loss(self, batch, preds=None):
        ...
        if self.manager is not None:
            self.manager.callbacks.on_after_compute_loss(batch['img'], preds, loss)
        return loss
    ...
    def set_compressor_manager(self, manager: neural_compressor.training.CompressionManager):
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        if self.manager is not None:
            loss = self.manager.callbacks.on_after_compute_loss(batch, preds, loss)
