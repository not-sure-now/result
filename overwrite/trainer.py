...
configs = [
    {  ## Example of a regular configuration
        ...
        "excluded_op_names": ["model.0.conv"],  # A list of modules that would not be pruned.
...
class BaseTrainer:
    ...
    def _do_train(self, world_size=1):
        ...
        if self.Com_Manager is not None:
            self.Com_Manager.callbacks.on_train_begin()
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                # intel neural compressor hook
                if self.Com_Manager is not None:
                    self.Com_Manager.callbacks.on_step_begin(i)
                ...
                if self.Com_Manager is not None:
                    self.Com_Manager.callbacks.on_before_optimizer_step()
                ...
                if self.Com_Manager is not None:
                    self.Com_Manager.callbacks.on_step_end()

            self.lr = {f'lr/pg{ir}': x['lr'] for ir, x in enumerate(self.optimizer.param_groups)}  # for loggers
            if self.Com_Manager is not None:
                self.Com_Manager.callbacks.on_epoch_end()
        ...
        if self.Com_Manager is not None:
            self.Com_Manager.callbacks.on_train_end()
